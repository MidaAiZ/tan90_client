...
$(function() {
    $.ajax({
        url: "http://115.159.188.200:8000/course_info//",
        type: "POST",
        data: {
...
function setupInfo(data) {
    ...
    $con.append($('<td></td>', {
        style: '',
        html: data.chapter_cnt
    }));
    $con.append($('<td></td>', {
        style: '',
        html: data.section_cnt
    }));
    $con.append($('<td></td>', {
        style: '',
        html: data.can_choose
    }));
    $con.append($('<td></td>', {
        style: '',
        html: data.already_choose
    }));
    $con.append($('<td></td>', {
        style: '',
        html: data.complete_cnt
    }));
    $con.append($('<td></td>', {
        style: '',
        html: data.content_cnt
    }));

    $('#c-chart').append($con);

    $("#b-value").text(data.already_choose);
    $("#l-value").text(data.complete_cnt);
...
var EditableTable = function() {
    ...
    return {
        ...
        init: function() {
            ...
            $('#heading').text(GetQueryString("course_name") + '：课程详情');
                                                    ...
                                                    if (data.code == 1000) {
                                                        ...
                                                        for (var i = 0; i < data.contents.length; i++) {
                                                            ...
                                                            $con.append($('<td></td>', {
                                                                style: '',
                                                                class: "content-id",
                                                                html: data.contents[i].id
                                                            }));
                                                            $con.append($('<td></td>', {
                                                                style: '',
                                                                class: "content-name",
                                                                html: data.contents[i].name
                                                            }));
                                                            $con.append($('<td></td>', {
                                                                style: '',
                                                                class: "chapter-name",
