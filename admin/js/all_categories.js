    ...
    return {
        ...
        init: function () {
                ...
                success: function(data){
                    ...
                    if(data.code==1000){
                        for (var i = 0; i < data.category.length; i++) {
                            var $cou= $('<tr></tr>');
                            $cou.append($('<td></td>',{class: "category-id",html:data.category[i].id}));
                            $cou.append($('<td></td>',{class: "category-name",html:data.category[i].name}));
            ...
            $('#editable-sample a.delete').live('click', function (e) {
                ...
                var f = confirm("确定要删除此分类吗 ?");
                if (f == false) {
                    return;
                }

                var delCatName = $(this).parent().prev().prev().text();
                
                var flag = false;
                //连接服务器删除
                $.ajax({
                        type: "POST",
                        url: "http://115.159.188.200:8000/del_category/",
                        data: "category_name="+delCatName,
                        dataType: "json",
                        async: false,
                        //下面2个参数用于解决跨域问题  
                        xhrFields: {
            ...
            $('#editable-sample a.edit').live('click', function (e) {
                ...
                var newCatName = $(this).parent().prev().find('input').val();

                //连接服务器提交
                $.ajax({
                        type: "POST",
                        url: "http://115.159.188.200:8000/add_category/",
